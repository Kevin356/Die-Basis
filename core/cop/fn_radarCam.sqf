...
if (_speed > _limit) then {
	if (side _driver == civilian) then {
		...
		if(license_civ_driver OR license_civ_truck ) then {
			...
			if(life_speedTicket <= 3) then {systemChat format["Das ist dein %1 Ticket fahr langsamer sonst verlierst du nochmehr Geld.",life_speedTicket];};
			if(life_speedTicket == 4) then {systemChat format["Das ist dein %1 Ticket beim Nächsten mal verlierst du deinen Führerschein. ",life_speedTicket];};
			if(life_speedTicket > 4 && life_speedTicket < 8) exitWith {
					
					systemChat format["Das ist dein %1 Ticket für dich.",life_speedTicket];
					...
					[4] remoteExecCall ["life_fnc_removeLicenses", _driver];
					[getPlayerUID _driver, name _driver, "120H"] remoteExecCall ["life_fnc_wantedAdd", 2];
					[2] call SOCK_fnc_updatePartial;
			};
			if(life_speedTicket >= 8) exitWith {
					
					systemChat format["Das ist dein %1 Ticket, nun können wir dir nicht mehr helfen. Kontaktiere am bessten mal einen Psychiater !!!",life_speedTicket];
					...
					[getPlayerUID _driver, name _driver, "120FS"] remoteExecCall ["life_fnc_wantedAdd", 2];
					[4] remoteExecCall ["life_fnc_removeLicenses", _driver];
					[2] call SOCK_fnc_updatePartial;
			};
			if(life_atmcash <= _ticket) then
			{
				if(life_atmcash <= _ticket) exitWith 
				{
					...
					[getPlayerUID _driver, name _driver, "120S"] remoteExecCall ["life_fnc_wantedAdd", 2];
				};
				life_atmcash = life_atmcash - _ticket;
				hintSilent parseText format ["<t color='#ffffff'><t size='2'><t align='center'>Speed Radar<br/><t color='#ff0000'><t align='center'><t size='1.5'>Speed: %1 km/h<br/><t color='#ffffff'><t align='center'><t size='1'>Speed Limit: %2 km/h<br/><t color='#ffffff'><t align='center'><t size='1'>Driver: %3<br/><t color='#ffffff'><t align='center'><t size='1'>Fine: $%4",round _speed,_limit,name _driver,[_ticket] call life_fnc_numberText];
				diag_log "Das Geld wurde von deinem Konto abgezogen";
				...
				[0] call SOCK_fnc_updatePartial;
					...
					if(life_speedCaught > 6) exitWith {
							...
							systemChat format["Das ist dein %1th TICKET hör auf zu Rasen!Denk an die Kinder!!",life_speedCaught];
							...
							[getPlayerUID _driver, name _driver, "120FSWL"] remoteExecCall ["life_fnc_wantedAdd", 2];
					...
					[getPlayerUID _driver, name _driver, "120WL"] remoteExecCall ["life_fnc_wantedAdd", 2];
					if(life_atmcash <= _ticketWL) then
					{
						...
						hintSilent parseText format ["<t color='#ffffff'><t size='2'><t align='center'>Speed Radar<br/><t color='#ff0000'><t align='center'><t size='1.5'>Speed: %1 km/h<br/><t color='#ffffff'><t align='center'><t size='1'>Speed Limit: %2 km/h<br/><t color='#ffffff'><t align='center'><t size='1'>Driver: %3<br/><t color='#ffffff'><t align='center'><t size='1'>Fine: $%4",round _speed,_limit,name _driver,[_ticketWL] call life_fnc_numberText];
						//diag_log "Das Geld wurde von deinem Konto abgezogen";//for logging
						[1] call SOCK_fnc_updatePartial;
					} else {
