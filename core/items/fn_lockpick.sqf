...
[player, "caralarm", 10] remoteExecCall ["life_fnc_playSound", -2];
...
if(!_isVehicle) then {
	...
	if(_dice < 30) then {
		...
		[_curTarget, "caralarm"] remoteExecCall ["life_fnc_say3D", -2];
		life_vehicles set[count life_vehicles,_curTarget];
		[getPlayerUID player, name player, "487"] remoteExecCall ["life_fnc_wantedAdd", 2];
	} else {
