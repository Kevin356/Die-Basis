...
if(_itemFilter == "vests" && vest player != "") exitWith{
...
if(_itemFilter == "attachments") then{
	if(player canAdd  _newItem) then{
		...
		_question1 = [format["Da Sie keinen Platz mehr im Rucksack oder in der Weste haben, spawnt der Aufsatz auf dem Boden!","Ok"]] call BIS_fnc_guiMessage;
			if(_question1) then 
			{
				_wh1 = createVehicle ["groundweaponholder", player modeltoworld [0,1,0], [], 0, "can_collide"]; 
				_wh1 addItemCargoGlobal [_newItem,1]; 
				life_is_processing = false;
			};
		};
};
if(_itemFilter == "magazines") then{
	if(player canAdd  _newItem) then{
		player addMagazine _newItem;
	}else{
		_question2 = [format["Da Sie keinen Platz mehr im Rucksack oder in der Weste haben, spawnt das Magazin auf dem Boden!","Ok"]] call BIS_fnc_guiMessage;
			if(_question2) then 
			{
				_wh2 = createVehicle ["groundweaponholder", player modeltoworld [0,1,0], [], 0, "can_collide"]; 
				_wh2 addMagazineCargoGlobal [_newItem,1]; 
				life_is_processing = false;
			};
		};
};
if(_itemFilter == "weapon") then {
    ...
    {
        player addWeaponGlobal _newItem;
    } else {
        _question = [format["Da Sie aktuell schon eine Waffe in der Hand haben, spawnt die gecraftete Waffe vor Ihnen am Boden!","Ok"]] call BIS_fnc_guiMessage;
        if(_question) then 
        {
            _wh = createVehicle ["groundweaponholder", player modeltoworld [0,1,0], [], 0, "can_collide"]; 
            _wh addWeaponCargoGlobal [_newItem,1]; 
            life_is_processing = false;
        };
    };
};
