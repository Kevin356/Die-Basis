...
switch (_code) do
{
	...
	case 57:
	{
		...
		if(_shift && {animationState player != "AovrPercMrunSrasWrflDf"} && {isTouchingGround player} && {stance player == "STAND"} && {speed player > 2} && {!life_is_arrested} && {(velocity player) select 2 < 2.5} && {time - jumpActionTime > 1.5}) then {
			...
			[player, false] remoteExecCall ["life_fnc_jumpFnc", -2]; //Global execution 
			_handled = true;
		};
	};
	...
	case 38: 
	{
		...
		if(_shift && playerSide in [west,independent]) then {
			if(vehicle player != player && (typeOf vehicle player) in ["C_Offroad_01_F","B_MRAP_01_F","B_MRAP_03_F","C_SUV_01_F","C_Hatchback_01_sport_F","C_Hatchback_01_F","B_Truck_01_transport_F","C_Offroad_01_repair_F"]) then {
	...
	case 39:
	{
		if(playerSide in [west] && vehicle player != player && !life_siren_active && ((driver vehicle player) == player)) then
		{
				...
				else
			{
				...
				if(playerSide == west) then {
					[_veh] remoteExec ["life_fnc_copSiren2", -2];
				} else {
					[_veh] remoteExec ["life_fnc_medicSiren", -2];
				};
			};
	...
	case 18:
	{
		...
		if !(license_civ_rebel) exitWith { hintSilent "Du brauchst ne Rebellenlizenz !"; };
		if(_shift && playerSide == civilian && !isNull cursorTarget && cursorTarget isKindOf "Man" && (isPlayer cursorTarget) && (side cursorTarget in [civilian,west]) && alive cursorTarget && cursorTarget distance player < 3.5 && !(cursorTarget getVariable "Escorting") && !(cursorTarget getVariable "restrained") && speed cursorTarget < 1) then
		{
			if([false,"zipties",1] call life_fnc_handleInv) then
			{
				...
				hintSilent "Du besitzt keine Kabelbinder";
			};
		};
	};
			...
			if(_veh isKindOf "House_F" && playerSide == civilian) then {
				...
				if(_veh in life_vehicles && player distance _veh < 8) then {
					if(_locked == 2) then {
						...
						[_veh] remoteExecCall ["life_fnc_LockCarSound", -2];
					} else {
						if(local _veh) then {
							_veh lock 2;
						} else {
						...
						[_veh] remoteExecCall ["life_fnc_UnLockCarSound", -2];
					};
				};
			};
		};
